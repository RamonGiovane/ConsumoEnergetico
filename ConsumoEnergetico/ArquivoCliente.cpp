...
unsigned int ArquivoCliente::tamanhoRegistro() {
	return TAMANHO_STRING * sizeof(char) * 6;
}
...
void ArquivoCliente::escreverObjeto(Cliente cliente) {
	...
	strncpy_s(registro.nome, TAMANHO_STRING, cliente.getNome().c_str(), cliente.getNome().length());
	strncpy_s(registro.bairro, TAMANHO_STRING, cliente.getBairro().c_str(), cliente.getBairro().length());
	strncpy_s(registro.CEP, TAMANHO_STRING, cliente.getCEP().c_str(), cliente.getCEP().length());
	strncpy_s(registro.cidade, TAMANHO_STRING, cliente.getCidade().c_str(), cliente.getCidade().length());
	strncpy_s(registro.rua, TAMANHO_STRING, cliente.getRua().c_str(), cliente.getRua().length());
	strncpy_s(registro.numero, TAMANHO_STRING, cliente.getNumero().c_str(), cliente.getNumero().length());
